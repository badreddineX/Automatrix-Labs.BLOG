// Builds a Metricool bulk-import CSV scheduling all 45 Australia posts to
// Instagram only, 2 posts/day, starting 2026-08-13 at 7PM and 9PM.
import { readFileSync, writeFileSync } from 'fs';

const COMMIT = '0a33ee7b52c74fa16ccca1b23643c268bb950b78';
const REPO = 'badreddineX/coastal-home-au.BLOG';
const IMG_DIR = 'social-posts/2026-08-13';
const BRAND = 'outdoorcoastal_home';
...
function igHashtags(post) {
  const specific = post.tags
    ...
    .filter(t => t.length > 3);
  const broad = ['#coastalhome', '#australianhomes', '#outdoorliving', '#hamptonsstyle', '#alfresco'];
...
writeFileSync('metricool-bulk-AUS-IG.csv', rows.join('\n') + '\n');
console.log(`✓ CSV — ${posts.length} posts, ${Math.ceil(posts.length / 2)} days`);

const scheduleLines = ['# Australia — Instagram Posting Schedule', '',
...
writeFileSync('SCHEDULE-AUS-IG-2-PER-DAY.md', scheduleLines.join('\n') + '\n');
